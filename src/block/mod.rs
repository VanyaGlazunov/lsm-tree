...
impl<'a> Iterator for BlockIterator<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let value = entry.copy_to_bytes(value_len);
...
mod tests {
    ...
    fn test_single_entry() {
        ...
        assert!(builder.add(Bytes::from("key"), Bytes::from("value")));
    ...
    fn test_multiple_entries() {
        let mut builder = BlockBuilder::new(1024);
        let entries = vec![
            (Bytes::from("apple"), Bytes::from("red")),
            (Bytes::from("banana"), Bytes::from("yellow")),
            (Bytes::from("cherry"), Bytes::from("red")),
        ];
    ...
    fn test_block_boundary() {
        ...
        assert!(builder.add(Bytes::from("k1"), Bytes::from("val1")));
        assert!(builder.add(Bytes::from("k2"), Bytes::from("val2")));

        assert!(!builder.add(Bytes::from("k3"), Bytes::from("val3")));
    ...
    fn test_seek_behavior() {
        ...
        iter.seek(b"d");
        assert_eq!(iter.next(), Some((Bytes::from("d"), Bytes::from("2"))));
    ...
    fn test_edge_cases() {
        let mut builder = BlockBuilder::new(1024);
        assert!(builder.add(Bytes::new(), Bytes::new()));

        let block = builder.build();
        let mut iter = block.iter();
        assert_eq!(iter.next(), Some((Bytes::new(), Bytes::new())));

        let mut builder = BlockBuilder::new(1024);
        let binary_key = Bytes::from(vec![0u8, 255u8]);
        let binary_val = Bytes::from(vec![1u8, 2u8, 3u8]);
    ...
    fn test_size() {
        ...
        builder.add(Bytes::from("k"), Bytes::from("v"));
        let expected_size = 2 + 1 + 2 + 1 + 2 + 2;
    ...
    fn test_large_entry() {
        ...
        assert!(builder.add(Bytes::from("very_long_key"), Bytes::from("very_long_value")));
